...
from config import INDUSTRY, REPORTS_DIR, ANOMALY_COL, logger
...
class AnomalyDetector:
    ...
    def run(self, columns: list = None) -> "AnomalyDetector":
        ...
        # Default to the configured anomaly column (total_amount); fall back to first numeric
        default_cols = [ANOMALY_COL] if ANOMALY_COL in self.df.columns else num_cols[:1]
        target_cols = columns if columns else default_cols
