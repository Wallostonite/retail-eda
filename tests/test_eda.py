...
def make_sample_df(rows: int = 50) -> pd.DataFrame:
    """Create a small, predictable retail DataFrame for testing."""
    np.random.seed(42)
    categories = ["Beauty", "Sports", "Fashion", "Auto", "Food", "Home", "Toys", "Electronics"]
    segments   = ["VIP", "Regular", "New", "At-risk", "Inactive"]
    statuses   = ["Delivered", "Shipped", "Processing", "Cancelled"]
    return pd.DataFrame({
        "order_id":          range(1, rows + 1),
        "order_date":        ["2024-01-15"] * rows,
        "total_amount":      np.random.uniform(4.67, 4999, rows).round(2),
        "order_status":      [statuses[i % len(statuses)] for i in range(rows)],
        "customer_segment":  [segments[i % len(segments)] for i in range(rows)],
        "product_category":  [categories[i % len(categories)] for i in range(rows)],
        "product_price":     np.random.uniform(10, 2000, rows).round(2),
        "seller_category":   [categories[(i + 2) % len(categories)] for i in range(rows)],
...
def test_anomaly_detector_flags_obvious_outlier():
    ...
    df.loc[0, "total_amount"] = 10_000_000   # obvious extreme outlier
...
def test_anomaly_detector_original_data_unchanged():
    ...
    df = make_sample_df()
    original_amount_0 = df.loc[0, "total_amount"]

    detector = AnomalyDetector(df)
    detector.run()

    assert df.loc[0, "total_amount"] == original_amount_0,         "AnomalyDetector should not modify the original DataFrame"
...
def test_detector_handles_zero_std_column():
    ...
    df = pd.DataFrame({
        ...
        "total_amount": np.random.uniform(10, 5000, 20),
    })
